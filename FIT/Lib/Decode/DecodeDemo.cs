...
namespace FITDecoder
{
	class Decoder
	{
		...
		static void Decode(string file_path)
		{
			...
			fitSource = new FileStream(file_path, FileMode.Open);
			Console.WriteLine("Opening {0}", file_path);
			...
			if (status == true)
			{
				...
				Console.WriteLine("Decoded FIT file {0}", file_path);
			}
			else
			{
				try
				{
					Console.WriteLine("Integrity Check Failed {0}", file_path);
